 ...
 */
abstract class AbstractOverrideEditAllHandler extends AbstractHandler
{
    /**
    ...
    protected function handleSubmit()
    {
        ...
        $sessionStorage->remove($dataDefinition->getName() . '.' . $this->getMode());

        $urlEvent = new GetReferrerEvent(false, $dataDefinition->getName());
     ...
     * @return void
     */
    private function makePropertyInvisibleByPalette(PropertyInterface $property)
    {
        $environment    = $this->getEnvironment();
        $dataDefinition = $environment->getDataDefinition();
        $palettes       = $dataDefinition->getPalettesDefinition();
    ...
    protected function updateErrorInformation(
        ...
        foreach (array_keys($modelError) as $modelId) {
            $newEditError = array(
                '<strong><a href="' . Environment::get('request') . '#pal_' . str_replace('::', '____', $modelId)
                . '">' . $modelId . '</a></strong>'
            );
     ...
     */
    protected function cloneCleanPropertyValueBag(PropertyValueBagInterface $propertyValueBag, ModelInterface $model)
    {
     ...
     * @return void
     */
    protected function resetPropertyValueErrors(PropertyValueBagInterface $propertyValueBag)
    {
     ...
     */
    protected function markPropertyInvalidErrorsByModel(
        PropertyValueBagInterface $propertyValueBag,
        ...
        foreach (array_keys($errorInformation) as $errorPropertyName) {
            array_map(
                function ($error) use (
                    ...
                    $inputField = 'ctrl_' . str_replace('::', '____', ModelId::fromModel($model)->getSerialized()) . '_'
                                  . $errorPropertyName;

                    $modelError[ModelId::fromModel($model)->getSerialized()][$errorPropertyName][] =
                        '<a href="' . Environment::get('request') . '#' . $inputField . '">No saved model['
                        . $model->getId() . ']. ' . $error . '</a>';
     ...
     * @return void
     */
    protected function updatePropertyValueBag(
        ModelInterface $model,
     ...
     * @SuppressWarnings(PHPMD.Superglobals)
     */
    protected function handleEditHandler(ModelInterface $model, PropertyValueBagInterface $propertyValueBag)
    {
        $environment     = $this->getEnvironment();
        $inputProvider   = $environment->getInputProvider();
        $editInformation = $GLOBALS['container']['dc-general.edit-information'];
     ...
     * @return void
     */
    protected function handleUnsetInputValueSubmitType(ModelInterface $model)
    {
        $environment   = $this->getEnvironment();
        $inputProvider = $environment->getInputProvider();
     ...
     * @SuppressWarnings(PHPMD.Superglobals)
     */
    protected function handleInputValues(ModelInterface $model)
    {
        $editProperties = $this->getEditPropertiesByModelId(ModelId::fromModel($model));
    ...
    protected function getCollectionFromSession()
    {
        ...
        foreach ($session['models'] as $modelId) {
            $dataConfig->setId(ModelId::fromSerialized($modelId)->getId());

            $result = $dataProvider->fetch($dataConfig);
    ...
    protected function injectSelectParentPropertyInformation(
        ...
        foreach ($palette->getLegends() as $legend) {
            ...
            foreach ($legend->getProperties() as $legendProperty) {
                ...
                foreach ($visibleCondition->getConditions() as $condition) {
                    ...
                    if (isset($properties[$condition->getPropertyName()])
                    ...
                    $properties[$condition->getPropertyName()]
        ...
        foreach ($properties as $propertyName => $informationProperty) {
            ...
            $information[] = sprintf(
                '<p class="tl_new">You must select the parent property "%s" for edit the property "%s".</p>'
                ,
                $labelParentProperty,
                $labelEditProperty
            );
    ...
    protected function injectSelectSubPropertiesInformation(
        ...
        foreach ($palette->getLegends() as $legend) {
            ...
            foreach ($legendProperties as $legendProperty) {
                ...
                foreach ($conditions->getConditions() as $condition) {
                    ...
                    if (isset($properties[$legendProperty->getName()])
                    ...
                    $properties[$legendProperty->getName()]
        ...
        foreach ($properties as $propertyName => $informationProperty) {
            ...
            $information[] = sprintf(
                '<p class="tl_new">You must select the property "%s" for edit it.</p>'
                ,
                $label
            );
